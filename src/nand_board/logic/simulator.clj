...
(defn set-val [state input-pin-id val]
  {:pre  [(s/valid? ::state-spec/state state)
   ...
   :post [(s/valid? ::state-spec/state %)]}
  (-> state
      (update :event-queue conj {:time (:time state) :pin-id input-pin-id :val val})
      process-events))
