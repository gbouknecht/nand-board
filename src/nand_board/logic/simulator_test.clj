...
(defn- get-vals [state pins]
  (->> (map #(vec [% (get-val state %)]) pins)
       (remove #(nil? (% 1)))
       (into {})))

(defn- get-vals-over-time [state pins]
  (->> state ticks (map #(get-vals % pins))))

(fact
  "initial state should have input pins set to 0"
  (let [board (-> (make-initial-board) (add-gates 2))
        [i1 i2 _ i4 i5 _ :as pins] (pins-for-gates board (last-added-gates board))
        state (make-initial-state board)]
    (get-vals state pins) => {i1 0 i2 0 i4 0 i5 0}))

(fact
  "initial state should have only unwired input pins set to 0"
  (let [board1 (-> (make-initial-board) (add-gates 2))
        [i1 i2 o3 i4 i5 _ :as pins] (pins-for-gates board1 (last-added-gates board1))
        board2 (-> board1 (add-wires [o3 i4]))
        state (make-initial-state board2)]
    (get-vals state pins) => {i1 0 i2 0 i5 0}))

(fact
  "propagation delay from input pin to output pin should be 2"
  (let [board (-> (make-initial-board) (add-gates 2))
        [i1 i2 o3 i4 i5 o6 :as pins] (pins-for-gates board (last-added-gates board))
        vals-over-time (-> (make-initial-state board) (get-vals-over-time pins))]
...
(fact
  ...
  (let [board1 (-> (make-initial-board) (add-gates 2))
        [i1 i2 o3 i4 i5 o6 :as pins] (pins-for-gates board1 (last-added-gates board1))
        board2 (-> board1 (add-wires [o3 i4]))
        vals-over-time (-> (make-initial-state board2) (get-vals-over-time pins))]
    vals-over-time => [{i1 0 i2 0 i5 0}
                       {i1 0 i2 0 i5 0}
                       {i1 0 i2 0 o3 1 i5 0}
                       {i1 0 i2 0 o3 1 i4 1 i5 0}
                       {i1 0 i2 0 o3 1 i4 1 i5 0}
                       {i1 0 i2 0 o3 1 i4 1 i5 0 o6 1}]))

(fact
  "gate should be a NAND"
  (let [board (-> (make-initial-board) (add-gates 1))
        [i1 i2 o3 :as pins] (pins-for-gates board (last-added-gates board))
    ...
    (get-vals state00 pins) => {i1 0 i2 0 o3 1}
    (get-vals state10 pins) => {i1 1 i2 0 o3 1}
    (get-vals state11 pins) => {i1 1 i2 1 o3 0}
    (get-vals state01 pins) => {i1 0 i2 1 o3 1}))
...
(fact
  ...
  (let [board (-> (make-initial-board) (add-gates 1))
    ...
    (get-vals state1 pins) => {i1 0 i2 0 o3 1}
    (get-vals state2 pins) => {i1 1 i2 1 o3 0}))

(fact
  "changes to input pins while a propagation is already in process should propagate correctly"
  (let [board (-> (make-initial-board) (add-gates 1))
        [i1 i2 o3 :as pins] (pins-for-gates board (last-added-gates board))
        vals-over-time (-> (make-initial-state board) tick (set-val i1 1) (set-val i2 1) (get-vals-over-time pins))]
...
(fact
  ...
  (let [board1 (-> (make-initial-board) (add-gates 2))
        ...
        state2 (-> state1 (set-val i1 1) (set-val i2 1) ticks last)]
    (get-vals state1 pins) => {i1 0 i2 0 o3 1 i4 1, i5 1, o6 0}
    (get-vals state2 pins) => {i1 1 i2 1 o3 0 i4 0, i5 0, o6 1}))
