...
(fact
  ...
  (let [event-queue (-> (make-event-queue) (add-event {:time 0 :pin (make-pin 0) :val 0}))]
    (count (-> event-queue (add-event {:time 1 :pin (make-pin 0) :val 0}))) => 2
    (count (-> event-queue (add-event {:time 0 :pin (make-pin 1) :val 0}))) => 2
    (count (-> event-queue (add-event {:time 0 :pin (make-pin 0) :val 1}))) => 1))
